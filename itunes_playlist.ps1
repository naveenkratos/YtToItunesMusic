...
if (Test-Path $txtFile) {
	...
	$expectedCount = 999 #Maximum 999 Songs
}
...
# Path of your config.ini file
$ConfigFile = Join-Path $scriptDir "config.ini"

# Hashtable to store playlist links
$PlaylistsUrls = @{}

# Read the Config file line by line to get YT playlist links
Get-Content $ConfigFile | ForEach-Object {
    ...
    # Match the pattern:  number = value
    if ($line -match '^\s*([0-9]+)\s*=\s*(.+)$') {
        $key   = $matches[1]
        $value = $matches[2].Trim()

        # Store in hashtable
        $PlaylistsUrls[$key] = $value
    }
}


#Loop PlaylistsUrls
foreach ($key in ($PlaylistsUrls.Keys | Sort-Object {[int]$_})) {
    
	$playlistURL = $PlaylistsUrls[$key]
	...
	Write-Host "Total Tracks in iTunes Library: $($libraryTracks.Count)"
	Write-Host "New tracks added to '$playlistName' Playlist: $added"
